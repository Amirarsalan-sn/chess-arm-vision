...
def augment_data():
    ...
    for sampleImgPath in glob.glob(os.path.join(samplePath, "*.jpg")):
        ...
        if filename.endswith('_r'):
            class_numb = class_dict[filename[0:-2]]
        else:
            class_numb = class_dict[filename]
        ...
        while count < output_per_sample:
            ...
            if flag:
                ...
                cv.imwrite(
                    os.path.join(outputfolder + "/images/", str(outputName + ".jpg")), finalImg
                )
                with open(
                        os.path.join(outputfolder + "/labels/", str(outputName + ".txt")), "w"
                ) as f:
                    details = (
                            f"{class_numb} "
                            + " ".join(
